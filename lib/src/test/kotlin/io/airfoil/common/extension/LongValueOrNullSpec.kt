...
class LongValueOrNullSpec : FunSpec({

    context("longValueOrNull") {
        test("SUCCESS - returns long for valid property") {
            TEST_GROUP_1.longValueOrNull("testLongConfig") shouldBe 123456789L
        }

        test("FAILURE - returns null for invalid property") {
            TEST_GROUP_1.longValueOrNull("invalidLongConfig") shouldBe null
        }
    }
