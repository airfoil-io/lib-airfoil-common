...
class BoolValueOrNullSpec : FunSpec({

    context("boolValueOrNull") {
        test("SUCCESS - returns boolean for valid property") {
            TEST_GROUP_1.boolValueOrNull("testBoolConfig") shouldBe true
        }

        test("FAILURE - returns null for invalid property") {
            TEST_GROUP_1.boolValueOrNull("invalidBoolConfig") shouldBe null
        }
    }
