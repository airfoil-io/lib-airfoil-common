...
class StringListOrErrorSpec : FunSpec({

    context("stringListOrError") {
        test("SUCCESS - returns string list for valid property") {
            TEST_GROUP_1.stringListOrError("testStringListConfig", "Property not found") shouldBe listOf(
                "string1",
                "string2",
                "string3",
            )
        ...
        test("FAILURE - throws exception for invalid property") {
            shouldThrow<ApplicationConfigurationException> {
                TEST_GROUP_1.stringListOrError("invalidStringListConfig", "Property not found")
            }
        }
    }
